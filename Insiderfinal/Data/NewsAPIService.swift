...
class NewsAPIService {
        ...
        do {
            ...
            if finalItems.isEmpty {
                ...
                if self.autoSaveToDatabase {
                    self.saveToDatabase(newsItems: finalItems, category: category)
                }
    ...
    private func saveToDatabase(newsItems: [NewsItem], category: NewsCategory) {
        // Calling the renamed NewsPersistenceManager
        NewsPersistenceManager.shared.saveNewsCards(newsItems) { result in
            switch result {
            case .success(let savedCards):
                print("✅ [AUTO-SAVE SUCCESS] Saved \(savedCards.count) items.")
