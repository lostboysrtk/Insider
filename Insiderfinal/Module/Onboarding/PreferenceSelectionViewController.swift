...
class PreferenceSelectionViewController: UIViewController {
    ...
    // MARK: - Properties
    private var selectedOption: TechLevel?
    // Removed currentStep and totalSteps properties
    ...
    enum TechLevel: String {
        ...
        var description: String {
            switch self {
            case .beginner:
                return "Just starting your tech adventure"
            case .intermediate:
                return "Building and learning actively"
            case .professional:
                return "Expert in your craft"
            }
        }
    ...
    private let pageTitle: UILabel = {
        ...
        lbl.alpha = 0 // for animation
        lbl.transform = CGAffineTransform(translationX: 0, y: -20) // for animation
        return lbl
    }()
    
    private let mainQuestionLabel: UILabel = {
        let lbl = UILabel()
        lbl.text = "How would you describe\nyour tech journey?"
    ...
    private let subtitleLabel: UILabel = {
        let lbl = UILabel()
        lbl.text = "This helps us personalize your experience"
    ...
    private let savePreferenceButton: UIButton = { // Renamed from continueButton
    ...
    private func setupUI() {
        // Removed progressContainer
        view.addSubview(pageTitle) // Added pageTitle to the view hierarchy
        ...
        NSLayoutConstraint.activate([
            // Position pageTitle at the top center
            pageTitle.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: 20),
            pageTitle.centerXAnchor.constraint(equalTo: view.centerXAnchor),

            mainQuestionLabel.topAnchor.constraint(equalTo: pageTitle.bottomAnchor, constant: 48), // Adjusted from progressContainer
        ...
        // Initial state for animation (includes pageTitle)
    ...
    // Removed setupProgressBar()
    
    private func setupOptionCards() {
        let options: [TechLevel] = [.beginner, .intermediate, .professional]
    ...
    private func setupActions() {
        savePreferenceButton.addTarget(self, action: #selector(savePreferenceTapped), for: .touchUpInside) // Renamed action
    }
    
    @objc private func cardTapped(_ gesture: UITapGestureRecognizer) {
        guard let tappedCard = gesture.view as? PreferenceCard else { return }
        
        let levels: [TechLevel] = [.beginner, .intermediate, .professional]
    ...
    @objc private func savePreferenceTapped() { // Renamed action
        ...
        UIView.animate(withDuration: 0.1, animations: {
            ...
            UIView.animate(withDuration: 0.1) {
                ...
                // Navigate to second preference screen
                print("Continue to next screen - Selected: \(selectedLevel.rawValue)")
                let nextVC = PreferenceSelection2ViewController()
                nextVC.modalPresentationStyle = .fullScreen
                self?.present(nextVC, animated: true) // Present the next screen
            }
        }
    }
    ...
    private func animateEntrance() {
        let elements = [pageTitle, mainQuestionLabel, subtitleLabel, optionsStackView, savePreferenceButton] // Added pageTitle
        
        for (index, element) in elements.enumerated() {
            UIView.animate(
                withDuration: 0.8,
                delay: 0.1 + Double(index) * 0.08, // Slightly faster staggered delay
...
// MARK: - PreferenceCard (No change needed)
class PreferenceCard: UIView {
    
    private let level: PreferenceSelectionViewController.TechLevel
    ...
    init(level: PreferenceSelectionViewController.TechLevel) {
        self.level = level
