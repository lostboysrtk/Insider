...
class DiscussionRowCell: UITableViewCell {
    ...
    func configure(with item: NewsItem, isStarted: Bool, isLast: Bool) {
        newsHeadline.text = item.title
        separatorLine.isHidden = isLast
        
        // Randomize replies to change when filters change
        let pool = isStarted ? [
            "How will this development impact the current industry standards and developers?",
            "I've tested this and the performance shift is quite significant for enterprise users.",
            "This is a major leap forward for the ecosystem's long-term compatibility."
        ] : [
            "I agree with the security concerns raised in the initial implementation phase.",
            "Interesting perspective, but the cost-to-benefit ratio seems a bit off here.",
            "Can we expect a follow-up patch for the documented stability issues?"
        ]
        discussionHeadline.text = pool.randomElement()
