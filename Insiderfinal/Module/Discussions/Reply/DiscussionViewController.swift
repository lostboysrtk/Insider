...
class DiscussionViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private var comments: [Comment] = [
        Comment(userName: "System_Architect", text: "The architectural shift here is massive. This API will redefine how we handle background tasks.", timeAgo: "2h", likes: 12, nestedReplies: [
            Comment(userName: "User_A", text: "Agreed, though I wonder about the migration path for older projects.", timeAgo: "1h", likes: 2),
            Comment(userName: "Me", text: "Backward compatibility is the main concern right now.", timeAgo: "45m", likes: 5)
        ], isExpanded: true),
        Comment(userName: "Dev_Emily", text: "I've been testing the beta; the performance gains are real.", timeAgo: "5h", likes: 8)
    ]
    ...
    @objc private func handleSendAction() {
        ...
        let newReply = Comment(userName: "Me", text: text, timeAgo: "now", likes: 0)
...
class DiscussionCardCell: UITableViewCell {
    ...
    private func createReplyBubble(for reply: Comment) -> UIView {
        ...
        let user = UILabel(); user.text = reply.userName == "Me" ? "You" : reply.userName
